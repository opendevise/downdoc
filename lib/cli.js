...
async function run () {
  ...
  if (values.postpublish) {
    ...
    await convertFile(inputPath, values.output)
...
function convertFile (inputPath, outputPath = toOutputPath(inputPath)) {
  ...
  return fsp.readFile(inputPath, 'utf8').then((input) => write(downdoc(input) + '\n', 'utf8'))
}

function hideInputFile (inputPath) {
  return gracefulStat(inputPath).then((stat) => (stat.isFile() ? fsp.rename(inputPath, '.' + inputPath) : undefined))
}

function restoreInputFile (inputPath, outputPath = toOutputPath(inputPath)) {
  return Promise.all([
    gracefulStat('.' + inputPath).then((stat) => (stat.isFile() ? fsp.rename('.' + inputPath, inputPath) : undefined)),
