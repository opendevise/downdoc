...
function parseArgs ({ args = process.argv.slice(2), options = {} }) {
  ...
  for (let i = -1, end = args.length - 1; i++ < end;) {
    ...
    if (arg in aliases ? (arg = aliases[arg]) : arg.length > 2 && arg.startsWith('--')) {
      ...
      const option = options[name] || { type: i === end ? 'boolean' : 'string' }
      const value = option.type === 'boolean' ? true : args[++i]
      option.multiple ? (values[name] ??= []).push(value) : (values[name] = value)
    } else {
