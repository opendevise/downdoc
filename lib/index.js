...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  let blockAttrs, blockTitle, cap, contentModel, hardbreak, indent, outdent, skipping, subs, match
  return lines
    .reduce((accum, line) => {
      while (true) {
        subs = undefined
        if (skipping) {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            inPara = false
            line = ''
          } else if ((!inPara || listStack.length) && chr0 in LIST_MARKERS && (match = ListItemRx.exec(line))) {
            let [, marker, text] = match
            subs = NORMAL_SUBS
            const ordered = marker[0] === '.' || (marker[0] === '<' && (marker = '<.>') && true)
            let list, siblingIdx
            if (listStack.length) {
              ~(siblingIdx = listStack.findIndex((it) => it.marker === marker)) && listStack.splice(siblingIdx + 1)
