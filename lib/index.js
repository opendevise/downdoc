...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  let blockAttrs, blockTitle, grab, hardbreakBeforeNext, match, skipping, subs, style, verbatim
  let indent = ''
    ...
    .reduce((accum, line, idx) => {
      while (true) {
        ...
        if (skipping) {
          ...
          inHeader = false
          inList ? (inPara = undefined) : (inPara = false) || (indent = inContainer.indent || '')
          ...
          let entering = true
          if (inPara !== false) listStack.splice(inList = (indent = inContainer.indent || '').undefined)
          if (inContainer !== (inPara = false) && line !== inContainer.delimiter) containerStack.push(inContainer)
          if ((grab = DELIMS[line]) === 'v' && (subs = ['callouts'])) {
            inContainer = { coseq: 1, delimiter: line, indent, inList, listStack, subs, verbatim: true }
            ...
            inContainer = { delimiter: line, indent, inList, listStack, verbatim: true }
            line = parseAttrlist(blockAttrs).get(1) === 'stem' ? (inContainer.cap = '```') + 'math' : undefined
            blockAttrs = blockTitle = undefined
          } else if (line === inContainer.delimiter) {
            ;({ cap: line, indent, inList, listStack } = inContainer)
            entering = inTable = blockAttrs = blockTitle = undefined
            inContainer = containerStack.length ? containerStack.pop() : false
          } else {
            inContainer = { delimiter: line, indent, inList, listStack, verbatim: false }
            if (grab === 't') {
          ...
          if (entering) inList = (listStack = []).undefined
        } else {
          const chr0 = line[0]
          if (inPara === undefined && !(inPara = false) && chr0 !== ' ') {
            ;(chr0 in LIST_MARKERS && ListItemRx.test(line)) || isDlistItem(line)
              ? accum.pop()
              : listStack.splice(inList = (indent = inContainer.indent || '').undefined)
          }
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (match.length === 4) {
              (blockAttrs && blockAttrs === 'qanda') || (inList && inList.marker === '.')
                ...
                : (text = '*' + marker + '*') && (marker = '-')
              hardbreakBeforeNext = match[2] ? (text += '\\\n' + match[2]) && undefined : 'pending'
            }
            ...
            if (!inList || inList.marker !== marker) {
              ...
              const selfIndent = inList ? inList.childIndent : (inContainer.indent || '')
              inList = { marker, indent: selfIndent, childIndent: selfIndent + childIndent }
              if (ordered) inList.numeral = 0
            }
            ...
            indent = inList ? inList.childIndent : inContainer.indent || ''
            if (blockAttrs) {
