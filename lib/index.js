...
module.exports = function downdoc (asciidoc, { attributes: initialAttrs = {} } = {}) {
  ...
  const lines = delete attrs.doctitle && asciidoc.split('\n')
        ...
        if (skipping || (skipping === false && line === 'endif::[]' && (skipping = line))) {
          ...
          if ((grab = DELIMS[line]) === 'v' && (attrs.$coseq = 1)) {
            ...
            if ((style = blockAttrs.get(1)) in ADMONS) {
              line = (inContainer.cap = '</dd></dl>') && '<dl><dt><strong>' + ADMONS[style] + ' ' + style +
                (blockTitle ? ': ' + blockTitle.text : '') + '</strong></dt><dd>\n'
              blockTitle = undefined
            } else if (blockAttrs.has('collapsible-option')) {
              line = attrs['markdown-collapsible-variant'] === 'spoiler'
                ? (inContainer.cap = '```') + (blockTitle ? 'spoiler ' + blockTitle.text : 'spoiler')
