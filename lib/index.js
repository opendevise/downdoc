        ...
        if (skipping) {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (row) {
              if (wrapped) {
                if (row.length) {
                  row.push(((last = row.pop())) ? `${hardbreak(last, '<br>')} ${wrapped}` : wrapped) // prettier-ignore
                } else {
                  const reopened = (last = accum.pop().split('\n'))[0].slice(0, -2).trimEnd()
                  last[0] = `${hardbreak(reopened, '<br>')} ${applySubs.call(substitutors, wrapped, subs)} |`
                  accum.push(last.length > 1 ? last.join('\n') : last[0])
                }
              }
              const cols = inTable.cols
              line = row.push(...cells) < cols ? undefined : `| ${row.splice(0, cols).join(' | ')} |`
            } else {
              inTable.currentRow = []
              inTable.cols ??= cells.length
              if (!(inTable.header ||= inTable.cols === cells.length && lines[idx + 1] === '')) {
          ...
          } else if (~line.indexOf(': ') && chr0 === chr0.toUpperCase() && (match = AdmonitionLabelRx.exec(line))) {
            subs = (inPara = true) && NORMAL_SUBS
...
function quotes (str) {
  ...
  if (~str.indexOf('\\') && ~str.indexOf('`')) str = str.replace(/`\S.*?\S`/g, (m) => m.replace(/\\(?![\s{])/g, ''))
