...
const AttributeReferenceRx = /(\\)?\{([\p{Ll}\d_][\p{Ll}\d_-]*)\}/gu
...
const EscapedPreprocessorDirectiveRx = /^\\(?:i(?:fn?def|nclude)::.+|endif::)\[.*\]$/
const InlineAnchorRx = /\[\[([\p{Alpha}_:][\p{Alpha}\d_\-.:]*)\]\]/u
...
module.exports = function downdoc (asciidoc, { attributes: { doctitle, ...initialAttrs } = {} } = {}) {
  ...
  const refs = {}
  const nrefs = new Map()
  let blockAttrs, blockTitle, chr0, grab, hardbreakAtNext, indent, listStack, match, skipping, subs, style, verbatim
  ...
  return lines
      ...
      while ((grab = match = style = subs = undefined) === undefined) {
        if (skipping) {
          ...
          if (line === inContainer.delimiter) {
            ...
            if (outdent) inContainer.outdent = Math.min(line.length - line.trimStart().length, outdent)
            if (~line.indexOf('{') || line[line.length - 1] === '>') subs = inContainer.subs
            if (grab > 8 && line[0] === '\\' && EscapedPreprocessorDirectiveRx.test(line)) line = line.slice(1)
          } else if (outdent && grab) inContainer.outdent = Math.min(line.length - line.trimStart().length, outdent)
        } else if (!line) {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (id) nrefs.set(title, refs[id] = { autoId, reftext: blockAttrs?.get('reftext'), title })
            blockAttrs = blockTitle = undefined
            ...
            } else if ((subs = NORMAL_SUBS) && hardbreakAtNext) {
    ...
    .replace(RewriteInternalXrefRx, (_, text, id) => {
      const { title = id, reftext = title, autoId = id } = refs[id] || nrefs.get(id) || {}
...
function quotes (str) {
  ...
  if (~str.indexOf('*')) str = str.replace(/(?:(?<!\w)\[[^[\]]+\]|(?<![\w\\]))\*(\S|\S.*?\S)\*(?!\w)/g, '**$1**')
  ...
  if (~str.indexOf(']_')) str = str.replace(/(?<!\w)\[[^[\]]+\](?=_\S(?:.*?\S)??_(?!\w))/g, '')
  ~str.indexOf('#') &&
    (str = str.replace(/(?:(?<!\w)(\[[^[\]]+\])|(?<![\w\\]))#(\S|\S.*?\S)#(?!\w)/g, (_, roles, text) =>
      roles ? (roles === '[.line-through]' ? '~~' + text + '~~' : text) : '<mark>' + text + '</mark>'
    ))
  return ~str.indexOf("'") ? str.replace(/\x60'(?!\x60)|(?<=[\p{Alpha}\d])'(?=\p{Alpha})/gu, '\u2019') : str
}
