...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  const convertInline = { attributes: subAttributeReferences.bind(attrs), callouts, macros: macros.bind(attrs), quotes }
      ...
      while (true) {
        ...
        if (skipping) {
          ...
          if (BLOCK_DELIMS[line] === 'verbatim') {
            ...
            inContainer = { delimiter: line, verbatim: true, subs: (subs = ['callouts']) }
            inPara = false
            let style, lang
