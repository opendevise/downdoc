        ...
        if (skipping) {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (row) {
              if (wrapped) {
                if (row.length) {
                  row[row.length - 1] = (last = row[row.length - 1]) ? `${hardbreak(last, '<br>')} ${wrapped}` : wrapped // eslint-disable-line no-cond-assign
                } else {
                  const reopened = (last = accum[accum.length - 1].split('\n'))[0].slice(0, -2).trimEnd()
                  last[0] = `${hardbreak(reopened, '<br>')} ${applySubs.call(substitutors, wrapped, subs)} |`
                  accum[accum.length - 1] = last.join('\n')
                }
              }
