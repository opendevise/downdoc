...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  const convertInline = { attributes: replaceAttributeReferences.bind(attrs), macros: macros.bind(attrs), quotes }
      ...
      while (true) {
        ...
        if (skipping) {
          ...
          if (line === inContainer.delimiter) {
            ...
            if (line[line.length - 1] === '>') line = line.replace(ConumRx, (_, spacer, num) => spacer + CONUMS[num])
          }
          ...
          if (BLOCK_DELIMS[line] === 'verbatim') {
            ...
            if (blockAttrs || (line === '----' && 'source-language' in attrs)) {
              ...
              if (~(blockAttrs.get('subs')?.indexOf('attributes') ?? -1)) inContainer.subs = subs = ['attributes']
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (!(name in attributes)) attrs[name] = value && convertInline.attributes(value)
            line = undefined
            ...
            if (currentRow) {
              if (appendToLast) {
                if (currentRow.length) {
                  ...
                  lastLines[0] = `${replaceLine} ${applySubs.call(convertInline, appendToLast, subs)} |`
        ...
        if (line !== undefined) {
          ...
          if (line) {
            if (subs && !(line = applySubs.call(convertInline, line, subs)) && !accum.length) return accum
...
function replaceAttributeReferences (str) {
  return ~str.indexOf('{') ? str.replace(AttributeReferenceRx, replaceAttributeReference.bind(this)) : str
}
