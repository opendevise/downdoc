...
const BLOCK_DELIMS = { '----': 'v', '....': 'v', '====': 'c', '****': 'c', ____: 'c', '|===': 't', '++++': 'p' }
const CONUMS = [1, 2, 3, 4, 5, 6, 7, 8, 9].reduce((m, i) => m.set(String(i), String.fromCharCode(10101 + i)), new Map())
...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  let blockAttrs, blockTitle, cap, contentModel, hardbreak, inList, indent, outdent, skipping, subs, match
  return lines
    .reduce((accum, line) => {
      while (true) {
        subs = undefined
        if (skipping) {
          ...
          inHeader = false
          inPara = inList ? undefined : !!(indent = undefined)
          ...
          if ((contentModel = BLOCK_DELIMS[line]) === 'v') {
            if (inContainer !== (inPara = false)) containerStack.push(inContainer)
            ...
            if (inContainer !== (inPara = false)) containerStack.push(inContainer)
            inContainer = { delimiter: line, verbatim: contentModel === 'p' }
            if (inContainer.verbatim) {
              line = parseAttrlist(blockAttrs).get(1) === 'stem' ? (inContainer.cap = '```') + 'math' : undefined
              blockAttrs = blockTitle = undefined
            } else if (contentModel === 't') {
              inTable = {}
          ...
          if (inPara === undefined && !(inPara = false) && chr0 !== ' ') {
            if ((chr0 in LIST_MARKERS && ListItemRx.test(line)) || isDlistItem(line)) {
              ...
              indent = undefined
              listStack.splice((inList = undefined))
            }
          }
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (!inList || inList.marker !== marker) {
              ...
              inList = ordered ? { marker, childIndent, numeral: 0 } : { marker, childIndent }
            }
