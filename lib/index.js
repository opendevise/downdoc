...
module.exports = function downdoc (asciidoc, { attributes: initialAttrs = {} } = {}) {
  ...
  let blockAttrs, blockTitle, chr0, grab, hardbreakNext, indent, listStack, match, next, subs, style, verbatim
  const containerStack = (indent = '') || (listStack = []).slice()
  const [nrefs, refs, skip, undef] = [new Map(), new Map(), [], () => undefined]
  return lines
    .reduce((accum, line, idx) => {
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skip.length && (match = skip[skip.length - 1] || (line === 'endif::[]' ? line : undefined))) {
          line === match ? skip.pop() : match === 'endif::[]' && /^ifn?def::.+\[\]$/.test(line) && skip.push(match)
          return accum
          ...
          return accum[accum.length - 1] ? accum.push(line) && accum : accum
          ...
          if (match[1]) {
            ...
            if (text ? (drop ? false : (line = text)) : !skip.push(drop && 'endif::[]')) continue // redo
          }
          line = undefined
          ...
          if (line === inContainer.delimiter) {
            ...
            const to = outdent && (match = inContainer.at) != null && (outdent + indent.length)
            if (to) for (let i = match, l = accum.length; ++i < l;) accum[i] = (indent + accum[i].slice(to)).trimEnd()
          ...
          if (chr0 === '/' && line[1] === '/') {
            line = line === '////' ? undef((inPara = !(skip.push(line)))) : undefined
            ...
            if (!inList || inList.marker !== marker) {
              if ((listStack.length && ~(match = listStack.findIndex((it) => it.marker === marker)))) {
                ...
                indent = (inList ? listStack[listStack.push(inList) - 1] : inContainer).childIndent || ''
        ...
        if (line) {
          ...
          if ((next &&= applySubs.call(attrs, next)) && (accum[accum.length - 1] += attrs.get('markdown-line-break'))) {
            accum.push(indent ? indent + next : next)
          }
        } else if (line === undefined) {
          return accum
        } else accum.push(indent ? indent.trimEnd() : line)
