...
module.exports = function downdoc (asciidoc, { attributes: initialAttributes = {} } = {}) {
  const attrs = Object.assign({}, ATTRIBUTES, delete initialAttributes.doctitle && initialAttributes)
        ...
        if (skipping) {
          ...
          if ((grab = DELIMS[line]) === 'v') {
            ...
            if ((grab = blockAttrs && blockAttrs.get('cols'))) {
              let cols = grab.split(/,|;/)
              if (~grab.indexOf('*')) cols = cols.flatMap((c) => ~c.indexOf('*') ? Array(parseInt(c, 10)).fill(c) : c)
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (!(name in initialAttributes)) attrs[name] = value && attributes.call(attrs, value)
            line = undefined
