  ...
  return lines
    .reduce((accum, line) => {
      while (true) {
        subs = undefined
        if (skipping) {
          skipping === line ? (line = skipping = undefined) : (line = undefined)
          ...
          if (line === inContainer.delimiter) {
            ...
            if ((contentModel = BLOCK_DELIMS[line]) === 'v') {
              inContainer = { autonum: 1, delimiter: line, verbatim: !(inPara = false), subs: (subs = ['callouts']) }
              let style, lang
              ...
              inContainer = { delimiter: line, verbatim: (inPara = false) }
              if (contentModel === 't') {
                inTable = {}
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (inHeader) {
              if (marker.length > 1 || 'doctitle' in attrs) {
                inHeader = false
