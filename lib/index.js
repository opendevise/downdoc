  ...
  return lines
    .reduce((accum, line) => {
      while (true) {
        ...
        if (skipping) {
          if (line === 'endif::[]' || line === '////' || (!inHeader && line === '|===')) {
            skipping = false
            if (!inHeader) while (accum[accum.length - 1] === '') accum.pop()
          }
          line = undefined
        } else if (verbatim) {
          if (line === '----') {
          ...
          if (chr0 === ':') {
            if (inHeader) {
              ...
              attrs[name] = replaceAttributeReferences(value, attrs)
            }
            line = undefined
          } else if (line === 'endif::[]') {
            line = undefined
            ...
            line = undefined
          } else if (chr0 === '=' && (!inHeader || line === lines[0])) {
            line = line.replace(SectionMarkerRx, (m) => '#'.repeat(m.length))
          } else if (inHeader) {
            if (!('author' in attrs) && line.match(AuthorInfoLineRx)) {
