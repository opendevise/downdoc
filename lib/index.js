...
const ADMONS = ((data) => Object.fromEntries(new Map(data.map((it) => it.split('=')))))(
  'CAUTION=\ud83d\udd25|IMPORTANT=\u2757|NOTE=\ud83d\udccc|TIP=\ud83d\udca1|WARNING=\u26a0\ufe0f'.split('|')
)
...
module.exports = function downdoc (asciidoc, { attributes: { doctitle, ...initialAttrs } = {} } = {}) {
  const attrs = Object.assign({ 'markdown-line-break': '\\', quotes: '<q> </q>' }, ATTRIBUTES, initialAttrs)
      ...
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skipping || (skipping === false && line === 'endif::[]' && (skipping = line))) {
          ...
          return inTable || !accum[accum.length - 1] ? accum : accum.push(line) && accum
            ...
            (match = PreprocessorDirectiveRx.exec(line)))) && (grab ? undef((line = line.slice(1))) : true)) {
          if (match[1]) {
            const [,, negated, name, text] = match
            skipping = (name in attrs ? negated : !negated) ? 'endif::[]' : false
            if (text && (skipping ? (skipping = undefined) : (line = (skipping = undefined) || text))) {
          ...
          if ((grab = DELIMS[line]) === 'v' && (attrs.$coseq = 1)) {
            ...
            if ((style = blockAttrs.get(1)) in ADMONS) {
              line = '<dl><dt><strong>' + ADMONS[style] + ' ' + style + (blockTitle ? ': ' + blockTitle.text : '') +
                '</strong></dt><dd>\n'
              inContainer.cap = '</dd></dl>'
            } else if (blockAttrs.has('collapsible-option') || (line = undefined)) {
              line = attrs['markdown-collapsible-variant'] === 'spoiler'
                ? (inContainer.cap = '```') + 'spoiler' + (blockTitle ? ' ' + blockTitle.text : '')
          ...
          if (opening !== (blockAttrs = undefined)) inList = undef((listStack = []))
        } else {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (row) {
              if (wrapped) {
                if (row.length && (inTable.hasWrappedRow = true)) {
                  ...
                  const opened = (grab = accum[accum.length - 1].split('\n'))[0].slice(0, -2).trimEnd()
                  grab[0] = hardbreak(opened, '<br>') + ' ' + applySubs.call(attrs, wrapped) + ' |'
                  accum[accum.length - 1] = grab.length > 1 ? grab.join('\n') : grab[0]
                }
              }
            ...
            if (dlist !== undefined) {
              (blockAttrs && blockAttrs.get(1) === 'qanda') || (inList && inList.dlist === 'qanda')
                ...
                : (text = '*' + marker + '*') && (marker = '-')
              ;(next = match[2]) || (hardbreakNext = 'pending')
            }
            ...
            if (!inList || inList.marker !== marker) {
              ...
              const listIndent = (attrs.$lindent ??= parseInt(attrs['markdown-list-indent'], 10)) || (ordered ? 3 : 2)
              inList = { marker, indent, childIndent: indent + ' '.repeat(listIndent), numeral: ordered && 0, dlist }
            }
            ...
            if (verbatim && indented) {
              ...
              accum[accum.length - 1] += attrs['markdown-line-break']
            } else if ((grab = accum[accum.length - 1] ?? '')[grab.length - 1] === '+') {
              accum[accum.length - 1] = hardbreak(grab, attrs['markdown-line-break'])
            }
          } else if (chr0 === '.') {
            subs = NORMAL_SUBS
            if (line.length > 1 && !(line[1] === '.' && line[2] === '.')) {
              const text = line[1] === '*' && line[line.length - 1] === '*' ? line.slice(2, -1) : line.slice(1)
              line = undef((blockTitle = { indent, text, subs }))
            }
          } else if (indented) {
            ...
            if ((inPara = true) && _chr0 === '$' && _line[1] === ' ') {
              indent = (inList || inContainer).childIndent || ''
              verbatim = { cap: indent + '```', close: () => undef(accum.push(verbatim.cap)), outdent, subs }
              line = '```console\n' + indent + line
            } else {
              indent = ((inList || inContainer).childIndent || '') + '    '
        ...
        if (line) {
          ...
          ;(next &&= applySubs.call(attrs, next)) && (accum[accum.length - 1] += attrs['markdown-line-break']) &&
            accum.push(indent ? indent + next : next)
        } else if (line === undefined || !accum.push(line)) return accum
...
function image (_, target, attrlist, _offset, _str, alt = attrlist.split(',')[0] || target.split('/').pop()) {
...
function writeBlockTitle (buffer, blockTitle, blockAttrs, attrs, refs) {
  const { id = blockAttrs?.get('id'), indent, text, subs } = blockTitle
  const title = applySubs.call(attrs, text, subs)
  buffer.push(indent + (id ? '<a name="' + (refs.set(id, { title }) && id) + '"></a>**' : '**') + title + '**', '')
}
