...
const ADMONS = ((data) => new Map(data.split('\n').map((it) => it.split(','))))(
  'CAUTION,\ud83d\udd25\nIMPORTANT,\u2757\nNOTE,\ud83d\udccc\nTIP,\ud83d\udca1\nWARNING,\u26a0\ufe0f'
)
const CONUMS = [...new Array(19)].reduce((m, _, i) => m.set(String(i + 1), String.fromCharCode(0x2460 + i)), new Map())
...
const AdmonitionLabelRx = new RegExp(`^(${[...ADMONS.keys()].join('|')}):(?=\\s)`)
...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  const listStack = Object.defineProperty([], 'indent', {
    get: () => (listStack.length ? listStack.reduce((buf, it) => buf + it.childIndent, '') : ''),
  })
  ...
  const conums = { get: (chr) => CONUMS.get(chr === '.' ? String(inContainer.autonum++) : chr) }
  substitutors.callouts = callouts.bind(conums)
  let blockAttrs, blockTitle, cap, contentModel, hardbreak, indent, match, skipping, subs, style, verbatim
  return lines
    .reduce((accum, line) => {
      while (true) {
        subs = undefined
        if (skipping) {
          ...
          inHeader = false
          inPara = inList ? (inList.containerStack.length ? false : undefined) : !!(indent = undefined)
          blockAttrs = blockTitle = verbatim = undefined
          ...
          if ((contentModel = DELIMS[line]) === 'v') {
            ...
            if (contentModel === 't') {
              ...
              if (ADMONS.has((style = (blockAttrs = parseAttrlist(blockAttrs)).get(1)))) {
                line = `<dl><dt><strong>${ADMONS.get(style)} ${style}</strong></dt><dd>\n`
          ...
          if (inPara === undefined && !(inPara = false) && chr0 !== ' ') {
            ;(chr0 in LIST_MARKERS && ListItemRx.test(line)) || isDlistItem(line)
              ? accum.pop()
              : listStack.splice((inList = indent = undefined))
          }
          if (chr0 === '/' && line[1] === '/') {
            ...
            subs = NORMAL_SUBS
            inPara = true
            line = `*${ADMONS.get((style = match[1]))} ${style}*${line.slice(style.length + 1)}`
