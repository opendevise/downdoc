...
function isDlistItem (str, capture, len = str.length) {
  if (str.indexOf(':: ') > 0) return DlistItemRx[capture ? 'exec' : 'test'](str)
  if (len > 2 && str[len - 1] === ':' && str[len - 2] === ':') return DlistItemRx[capture ? 'exec' : 'test'](str)
}
