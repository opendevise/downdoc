      ...
      while (true) {
        ...
        if (skipping || line === 'endif::[]') {
          ...
          if (chr0 === '/' && line.charAt(1) === '/') {
            ...
            line = ''
          } else if ((chr0 === '*' || chr0 === '.') && ListMarkerRx.test(line)) {
            ...
            line = ordered ? `${(list.numeral += 1)}. ${text.join(' ')}` : `* ${text.join(' ')}`
          } else if (chr0 === '.') {
          ...
          } else if (chr0 === ':' && (m = line.match(AttributeEntryRx))) {
            line = undefined
          } else if (line === 'toc::[]') {
            line = undefined
          ...
          } else if (!prev && ~line.indexOf(': ') && (m = line.match(AdmonitionLabelRx))) {
            subs = NORMAL_SUBS
            const label = m[1]
