...
function isDlistItem (str, capture) {
  if (!~str.indexOf('::') || (!~str.indexOf(':: ') && str[str.length - 1] !== ':')) return
  return DlistItemRx[capture ? 'exec' : 'test'](str)
}

function isHeading (str, acceptAll, blockAttrs, marker, title, spaceIdx = str.indexOf(' ')) {
  if (!~spaceIdx || spaceIdx > 6 || str.slice(0, spaceIdx) !== (marker = '='.repeat(spaceIdx))) return
