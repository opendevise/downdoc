...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  let blockAttrs, blockTitle, contentModel, hardbreakBeforeNext, last, match, skipping, subs, style, verbatim
  let indent = ''
  return lines
      ...
      while (true) {
        ...
        if (skipping) {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            blockAttrs = parseAttrlist(style = blockAttrs)
            let discrete, id, titleWords
            if ((discrete = style ? blockAttrs.get(1) === 'discrete' : false) || inContainer === false) {
...
function macros (str) {
  ...
  if (!(hasColon && ~str.indexOf('xref:')) && !~str.indexOf('<<')) return str
  return str.replace(XrefRx, (_, _id, _text = '', id = _id, text = _text) =>
...
function quotes (str) {
  ...
  if (~str.indexOf('`+') || (~str.indexOf('\\') && ~str.indexOf('`'))) {
    str = str.replace(/`(\+)?(\S|\S.*?\S)\1`/g, (_, pass, text) => {
      if (pass) return '`' + (~text.indexOf('{') ? text.replace(/\{(?=[a-z])/gu, '\\{') : text) + '`'
