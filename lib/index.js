...
module.exports = function downdoc (asciidoc, { attributes: initialAttrs = {} } = {}) {
  ...
  const [nrefs, refs, undef] = [new Map(), new Map(), () => undefined]
  return lines
      ...
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skipping || (skipping === false && line === 'endif::[]' && (skipping = line))) {
          ...
          if (match[1]) {
            ...
            if (text ? (drop ? false : (line = text)) : (skipping = drop && 'endif::[]') === true) continue // redo
          }
          line = undefined
          ...
          if (chr0 === '/' && line[1] === '/') {
            line = line === '////' ? undef((inPara = !(skipping = line))) : undefined
            ...
            if (!inList || inList.marker !== marker) {
              if (inList && (~(match = listStack.findIndex((it) => it.marker === marker)) || !listStack.push(inList))) {
