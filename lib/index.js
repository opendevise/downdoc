...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  let blockAttrs, blockTitle, cap, contentModel, hardbreak, inList, inTable, indent, outdent, skipping, subs, match
  return lines
    .reduce((accum, line) => {
      while (true) {
        subs = undefined
        if (skipping) {
          ...
          if (inContainer !== (inPara = false) && inContainer.delimiter !== line) containerStack.push(inContainer)
          ...
          } else if (inContainer.delimiter === line) {
            line = inContainer.cap
