      ...
      while (true) {
        ...
        if (skipping) {
          ...
          if ((grab = DELIMS[line]) === 'v') {
            ...
            const cols = blockAttrs && blockAttrs.get('cols')
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            const discrete = blockAttrs ? blockAttrs.get(1) === 'discrete' : !(blockAttrs = { get: () => undefined })
...
function macros (str, hasColon) {
  ...
  return str.replace(XrefMacroRx, (_, p, s = '', id, text) => '[' + text + '](' + (p ? p + s : '#!' + id) + ')')
}
