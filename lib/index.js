            ...
            if (currentRow) {
              if (wrapped) {
                if (currentRow.length) {
                  ...
                  currentRow[currentRow.length - 1] = last ? `${hardbreak(last, '<br>')} ${wrapped}` : wrapped
                } else {
                  ;(last = accum[accum.length - 1].split('\n'))[0] =
                    hardbreak(last[0].slice(0, -2).trimEnd(), '<br>') +
                    ` ${applySubs.call(substitutors, wrapped, subs)} |`
