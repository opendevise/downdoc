...
const BoldPhraseRx = /(?<![\p{L}\d_\\])(?:\[[^[]+?\])?(\*\S(?:.*?\S)?\*)(?![\p{L}\d_])/gu
...
const ItalicPhraseModifierRx = /(?<![\p{L}\d_\\])\[[^[]+?\](?=_\S(?:.*?\S)??_(?![\p{L}\d_]))/gu
const LinkMacroRx = /(?:link:(?!:)|(https?:\/\/))([^\s[\\]+)\[(|.*?[^\\])\^?\]|(\\)?((https?:\/\/)([^\s[\]]+))(?=\s|$)/g
const ListItemRx = /^(\*+|\.+|<[1-9.]>|-(?! -)) (\S.*)/s
const MarkedPhraseRx = /(?<![\p{L}\d_\\])(?:\[((\.line-through)|[^[]+?)\])?#(\S(?:.*?\S)?)#(?![\p{L}\d_])/gu
...
function quotes (str, idx) {
  ...
  if (~str.indexOf('"`')) str = str.replace(/"`(\S(?:.*?\S)?)`"/g, (this.$q ??= this.quotes.split(' ', 2).join('$1')))
  if (~str.indexOf('`') && (hasLt || ~str.indexOf('`+') || ~str.indexOf('\\'))) {
    str = str.replace(/`(\+)?(\S(?:.*?\S)?)\1`/g, (_, pass, text) => {
