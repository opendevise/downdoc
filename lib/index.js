...
const ListItemRx = /^(\.+|\*+) (\S.*)$/
const InlineMacroRx = /(?:link:|(image:(?!:))|(https?:))([^[\\]+)\[(|.*?[^\\])\]/g
    ...
    .reduce((accum, line) => {
      while (true) {
        ...
        if (skipping || line === 'endif::[]') {
          ...
          subs = undefined
          if (inBlock == null) {
            if ((chr0 === '*' || chr0 === '.') && ListItemRx.test(line)) {
              accum.pop()
            } else {
              indent = undefined
              listStack.length = 0
          ...
          if (chr0 === '/' && line.charAt(1) === '/') {
            ...
            line = undefined
          } else if (chr0 === '=' && !inBlock && (m = line.match(SectionTitleRx))) {
            let [, marker, title] = m
            if (inHeader) {
          ...
          } else if (chr0 === ':' && !inBlock && (m = line.match(AttributeEntryRx))) {
            const [, name, value = ''] = m
            ...
            indent = listStack.indent.slice(0, -list.indent.length)
            ...
            const label = m[1]
            inBlock = true
            line = `${ADMONS[label]} *${label}:*${line.slice(label.length + 1)}`
          } else if (chr0 === 'i' && line.startsWith('image::') && (m = line.match(BlockImageMacroRx))) {
            const [, target, text] = m
            subs = ['attributes']
...
function macros (str) {
  return str
    .replace(InlineMacroRx, (_, img = '', scheme = '', target, text) => {
