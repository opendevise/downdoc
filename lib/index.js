...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  let blockAttrs, blockTitle, cap, contentModel, hardbreak, inList, inTable, indent, match, skipping, subs, verbatim
  return lines
    .reduce((accum, line) => {
      while (true) {
        subs = undefined
        if (skipping) {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            const label = match[1]
            inPara = true
            line = `${ADMONS.get(label)} *${label}:*${line.slice(label.length + 1)}`
