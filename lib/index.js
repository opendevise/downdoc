      ...
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skipping || (skipping === false && line === 'endif::[]' && (skipping = line))) {
          ...
          if (inPara === undefined && !(inPara = false) && (_chr0 = indented ? (_line = line.trimStart())[0] : chr0)) {
            ;((_chr0 in LIST_MARKERS && ListItemRx.test(_line)) || isDlistItem(_line)) && inList
              ? accum.pop()
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            return accum.push(line && line.trimEnd()) && accum
              ...
              ((_chr0 in LIST_MARKERS && (match = ListItemRx.exec(_line))) || (match = isDlistItem(_line, 'exec')))) {
            ...
            subs = NORMAL_SUBS
            if (verbatim && indented && (subs = verbatim.subs) !== null) {
...
function isDlistItem (str, mode = 'test') {
  if (~str.indexOf('::') && (~str.indexOf(':: ') || str[str.length - 1] === ':')) return DlistItemRx[mode](str)
}
...
function quotes (str, idx) {
  ...
  return ~str.indexOf("'") ? str.replace(/([\p{L}\d])'(?=\p{L})|\x60'(?!\x60)/gu, '$1\u2019') : str
}
