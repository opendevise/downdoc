...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  let blockAttrs, blockTitle, contentModel, hardbreakBeforeNext, indent, last, match, skipping, subs, style, verbatim
  const containerStack = []
  const listStack = Object.defineProperty([], 'indent', { writable: true, value: (indent = '') })
  return lines
    .reduce((accum, line, idx) => {
      while (true) {
        ...
        if (skipping) {
          ...
          blockAttrs = blockTitle = (indent = '').undefined
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            const discrete = (blockAttrs = parseAttrlist(last = blockAttrs)) && last && blockAttrs.get(1) === 'discrete'
            ...
            if (row) {
              if (wrapped) {
                if (row.length && (inTable.hasWrappedRow = true)) {
                  row.push(((last = row.pop())) ? `${hardbreak(last, '\\\n')} ${wrapped}` : wrapped) // prettier-ignore
                } else {
                  const reopened = (last = accum.pop().split('\n'))[0].slice(0, -2).trimEnd()
                  last[0] = `${hardbreak(reopened, '<br>')} ${applySubs.call(substitutors, wrapped, NORMAL_SUBS)} |`
                  accum.push(last.length > 1 ? last.join('\n') : last[0])
                }
              }
            ...
            verbatim = verbatim?.close()
            subs = (inPara = true) && NORMAL_SUBS
            line = ordered ? `${++inList.numeral}. ${text}` : '* ' + text
          } else if ((!inPara || inList) && (match = isDlistItem(line, true))) {
            const [, term, desc] = match
            line = blockAttrs === 'qanda' || (inList && inList.marker === '.') ? `. _${term}_` : `- *${term}*`
            hardbreakBeforeNext = desc ? (line += '\\\n' + desc) && undefined : 'pending'
            continue // reprocess line
          } else if (inPara) {
            subs = NORMAL_SUBS
            if (verbatim && chr0 === ' ') {
        ...
        if (blockTitle) {
          const { id, indent: lpad, text: titleText, subs: titleSubs } = blockTitle
          const title = applySubs.call(substitutors, titleText, titleSubs)
          accum.push(id ? `${lpad}<a name="${id}"></a>**${(refs[id] = { title }).title}**` : `${lpad}**${title}**`, '')
    ...
    .trimEnd()
    .concat(((last = verbatim?.cap)) ? '\n' + last : '') // prettier-ignore
}
...
function macros (str) {
  const hasColon = ~str.indexOf(':')
  if (hasColon && ~str.indexOf('image:')) str = str.replace(InlineImageMacroRx, image.bind(this))
  ...
  return str.replace(XrefMacroRx, (_, id, inter, h, text) => `[${text}](${inter || h ? '' : '#!'}${id})`)
}
