      ...
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skipping || (skipping === false && line === 'endif::[]' && (skipping = line))) {
          ...
          if (line === inContainer.delimiter) {
            const left = inContainer.outdent
            if (left) for (let i = inContainer.at, l = accum.length; ++i < l;) accum[i] = accum[i].slice(left)
            ...
            inContainer.outdent &&= Math.min(grab - line.trimStart().length, inContainer.outdent)
            if (~line.indexOf('{') || line[grab - 1] === '>') subs = inContainer.subs
          } else if (grab) inContainer.outdent &&= Math.min(grab - line.trimStart().length, inContainer.outdent)
