...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  substitutors.callouts = callouts.bind({ get: (chr) => CONUMS[chr === '.' ? inContainer.conumSeq++ : chr] })
    ...
    .reduce((accum, line, idx) => {
      while (true) {
        ...
        if (skipping) {
          ...
          if (inList) inList.containerStack.length ? (indent = inList.childIndent) : (inPara = undefined)
          if (inTable || !accum[accum.length - 1]) line = undefined
        } else if (!inHeader && line in DELIMS) {
          if (inPara === undefined) listStack.splice(inList = (indent = '').undefined)
          if (inContainer !== (inPara = false) && line !== inContainer.delimiter) containerStack.push(inContainer)
          if ((grab = DELIMS[line]) === 'v') {
            inContainer = { conumSeq: 1, delimiter: line, verbatim: true, subs: (subs = ['callouts']) }
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (!inList || inList.marker !== marker) {
              ...
              const selfIndent = inList ? inList.childIndent : ''
              inList = { marker, containerStack: [], indent: selfIndent, childIndent: selfIndent + childIndent }
            ...
            indent = inList ? inList.childIndent : ''
            if (blockAttrs) {
