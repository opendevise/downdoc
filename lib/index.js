        ...
        if (skipping) {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (currentRow) {
              ...
              line =
                currentRow.push(...cells) < inTable.cols
                  ? undefined
                  : `| ${currentRow.splice(0, inTable.cols).join(' | ')} |`
            } else {
              inTable.currentRow = []
            ...
            subs = ['attributes']
            line = `![${text.split(',')[0] || target.split('/').pop()}](${target})`
          } else {
...
function macros (str) {
  return str
    ...
    .replace(LinkMacroRx, (_, scheme = '', url, text, esc, bareUrl, bareScheme, bareUrlWithoutScheme) => {
      ...
      if (bareUrl) return this['hide-uri-scheme'] == null ? bareUrl : `[${bareUrlWithoutScheme}](${bareUrl})`
      return `[${text || (this['hide-uri-scheme'] == null ? scheme : '') + url}](${scheme + url})`
    })
