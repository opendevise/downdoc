...
const ATTRIBUTES = JSON.parse(
  '{"empty":"","idprefix":"_","idseparator":"_","markdown-line-break":"\\\\","nbsp":"&#160;","quotes":"<q> </q>",' +
    '"sp":" ","vbar":"|","zwsp":"&#8203;"}'
)
...
module.exports = function downdoc (asciidoc, { attributes: initialAttrs = {} } = {}) {
  const attrs = new Map(Object.entries(Object.assign({ doctitle: undefined }, ATTRIBUTES, initialAttrs)))
  const lines = attrs.delete('doctitle') && asciidoc.split('\n')
      ...
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skipping || (skipping === false && line === 'endif::[]' && (skipping = line))) {
          ...
          if ((grab = DELIMS[line]) === 'v' && (attrs.coseq = 1)) {
            ...
            if ((style = blockAttrs.get(1)) in ADMONS && (inContainer.cap = '</dd></dl>')) {
              const anchor = blockAttrs.has('id') ? '<a name="' + blockAttrs.get('id') + '"></a>' : ''
              blockTitle ? (line = ': ' + blockTitle.text) && (blockTitle = undefined) : (line = '')
              line = '<dl><dt><strong>' + anchor + ADMONS[style] + ' ' + style + line + '</strong></dt><dd>\n'
            } else if (blockAttrs.has('collapsible-option')) {
              line = attrs.get('markdown-collapsible-variant') === 'spoiler'
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if ((inHeader = attrs.has('doctitle'))) {
              if (!attrs.has('author') && AuthorInfoLineRx.test(line)) {
                ...
                revnumber && attrs.set('revnumber', revnumber)
                revdate && attrs.set('revdate', revdate)
              } else inHeader = false
            }
...
function quotes (str, idx) {
  ...
  if (~str.indexOf('"`')) {
    str = str.replace(/"`(\S|\S.*?\S)`"/g, (this.q ??= this.get('quotes').split(' ').slice(0, 2).join('$1')))
  }
