...
const BLOCK_DELIMITERS = { '----': 'verbatim', '....': 'verbatim', '====': true, '****': true, ____: true }
...
const SubsAttrWithAttributesRx = /^subs=(")?\+?attributes\+?\1$/
...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  const refs = {}
  let blockAttrlist, blockTitle, cap, hardbreak, indent, outdent, skipping, subs, match
  return lines
    .reduce((accum, line) => {
      while (true) {
        if (skipping) {
        ...
        } else if (!inHeader && line in BLOCK_DELIMITERS) {
          if (BLOCK_DELIMITERS[line] === 'verbatim') {
            if (inContainer !== false) containerStack.push(inContainer)
            inContainer = { delimiter: line, verbatim: true }
            inPara = false
            let style, lang
            if (blockAttrlist || (line === '----' && 'source-language' in attrs)) {
              const blockAttrs = blockAttrlist?.split(',') || ['source']
              if ((style = blockAttrs[0] || (line === '----' ? 'source' : 'literal')) === 'source') {
                if (!(lang = blockAttrs[1]?.trimStart()) || ~lang.indexOf('=')) lang = attrs['source-language']
              } else if (line === '....' && style !== 'listing' && style !== 'literal') {
                lang = style
              }
              const subsAttr = blockAttrs.find((it) => it.startsWith('subs='))
              if (subsAttr && SubsAttrWithAttributesRx.test(subsAttr)) subs = ['attributes']
            ...
            inPara = false
            if (line === '====' && hasOption(blockAttrlist, 'collapsible')) {
            ...
            blockAttrlist = blockTitle = undefined
          }
        } else if (line.length) {
          subs = undefined
          ...
          if (chr0 === '/' && line.charAt(1) === '/') {
            ...
            inPara = !(skipping = '|===')
            line = blockAttrlist = blockTitle = undefined
            ...
            subs = NORMAL_SUBS
            if (line.length > 1 && !(line.charAt(1) === '.' && line.charAt(2) === '.')) {
              line = line.charAt(1) === '*' && line.charAt(line.length - 1) === '*' ? line.slice(2, -1) : line.slice(1)
          ...
          blockAttrlist = blockTitle = outdent = undefined
          cap && accum.push(cap) && (cap = undefined)
        }
        if (line !== undefined) {
          if (blockTitle) {
...
function hasOption (attrlist, name) {
  if (!attrlist || !(attrlist = attrlist.split(/,|\[|\w[\w-]*=/, 1)[0])) return false
  return attrlist.split(/(?=[#.%])/).includes('%' + name)
}
...
function quotes (str) {
  return str
    ...
    .replace(/(?:\[([^[\]]+)\]|)(?<!\\)#(\S|\S.*?\S)#/g, (_, attrlist, text) => attrlist ? text : `<mark>${text}</mark>`)
