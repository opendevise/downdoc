...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  let blockAttrs, blockTitle, cap, contentModel, hardbreakState, match, skipping, subs, style, verbatim
  let indent = ''
  return lines
    .reduce((accum, line) => {
      while (true) {
        ...
        if (skipping) {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (currentRow) {
              if (appendToLast) {
                if (currentRow.length) {
                  ...
                  currentRow.push(lastCell ? `${hardbreak(lastCell, '<br>')} ${appendToLast}` : appendToLast)
                } else {
                  const lastLines = accum.pop().split('\n')
                  const replacementLine = hardbreak(lastLines[0].slice(0, -2).trimEnd(), '<br>')
                  lastLines[0] = `${replacementLine} ${applySubs.call(substitutors, appendToLast, subs)} |`
            ...
            hardbreakState = desc ? undefined : 'pending'
            line = `- *${term}*${desc ? '\\\n' + desc : ''}`
            continue // reprocess line
          } else if (inPara) {
            if (verbatim && chr0 === ' ') {
              ...
              subs = NORMAL_SUBS
              let lastLine = accum[accum.length - 1]
              hardbreakState
                ? (accum[accum.length - 1] = lastLine + '\\')
                : (lastLine &&= hardbreak(lastLine, '\\', undefined)) && (accum[accum.length - 1] = lastLine)
            }
          } else if (chr0 === '.') {
            subs = NORMAL_SUBS
        ...
        hardbreakState &&= hardbreakState === 'pending' || undefined
        accum.push(line)
        return accum
      }
    }, [])
