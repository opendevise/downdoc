    ...
    .reduce((accum, line, idx) => {
      while (true) {
        ...
        if (skipping) {
          ...
          if (inPara !== false) inPara = !listStack.splice(inList = (indent = inContainer.indent || '').undefined)
          const closing = inContainer === false || (line !== inContainer.delimiter && containerStack.push(inContainer))
            ? (inContainer = { delimiter: line, indent, inList, listStack, verbatim: false }) && false
            : true
          ...
          if (!closing) inList = (listStack = []).undefined
        } else {
          const chr0 = line[0]
