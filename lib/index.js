...
module.exports = function downdoc (asciidoc, { attributes: initialAttrs = {} } = {}) {
  ...
  const [containerStack, nrefs, refs, skip, undef] = [(listStack = []).slice(), new Map(), new Map(), [], () => {}]
  return lines
    .reduce((accum, line, idx) => {
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skip.length && (match = skip[skip.length - 1] || (line === 'endif::[]' ? line : undefined))) {
          if (line === match) return undef(skip.pop()) || accum
          if (match === 'endif::[]' && line.startsWith('if') && /^ifn?def::.+\[\]$/.test(line)) skip.push(match)
          return accum
