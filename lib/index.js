      ...
      while (true) {
        ...
        if (skipping || line === 'endif::[]') {
          ...
          if (chr0 === '/' && line.charAt(1) === '/') {
            ...
            line = undefined
          } else if (chr0 === 'i' && ~line.indexOf('def::') && (match = line.match(ConditionalDirectiveRx))) {
            ...
            line = undefined
          } else if (chr0 === '=' && !inBlock && (match = line.match(SectionTitleRx))) {
          ...
          } else if (chr0 === ':' && !inBlock && (match = line.match(AttributeEntryRx))) {
            ...
            if ((inHeader = 'doctitle' in attrs)) {
              ...
              } else if (!('revdate' in attrs) && !('revnumber' in attrs) && (match = line.match(RevisionInfoLineRx))) {
                console.dir(match)
            ...
            inBlock = false
            line = ''
          } else if (chr0 in LIST_MARKERS && (!inBlock || listStack.length) && (match = line.match(ListItemRx))) {
          ...
          } else if (chr0 === chr0.toUpperCase() && ~line.indexOf(': ') && (match = line.match(AdmonitionLabelRx))) {
            subs = NORMAL_SUBS
            const label = match[1]
            inBlock = true
            line = `${ADMONITION_ICONS[label]} *${label}:*${line.slice(label.length + 1)}`
          } else if (chr0 === 'i' && line.startsWith('image::') && (match = line.match(BlockImageMacroRx))) {
