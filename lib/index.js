      ...
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skipping || (skipping === false && line === 'endif::[]' && (skipping = line))) {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            const [wrapped, ...cells] = ~line.indexOf('|') ? line.split(CellDelimiterRx) : [line]
            if (row) {
              if (wrapped) {
...
function quotes (str, idx) {
  ...
  if (~str.indexOf('`') && ((idx = ~str.indexOf('"`')) || true)) {
    if (idx) str = str.replace(/"`(\S|\S.*?\S)`"/g, (this.q ??= this.get('quotes').split(' ').slice(0, 2).join('$1')))
