    ...
    .reduce((accum, line, idx) => {
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skipping || (skipping === false && line === 'endif::[]' && (skipping = line))) {
          ...
          if (opening !== undefined) inList = undef((listStack = []))
        } else {
            ...
            if (row) {
              if (wrapped) {
                if (row.length && (inTable.hasWrappedRow = true)) {
                  ...
                  grab[0] = hardbreak(opened, '<br>', opened, ' ') + applySubs.call(attrs, wrapped) + ' |'
        ...
        if (blockTitle) {
          const { id = blockAttrs?.get('id'), indent: pd, text: titleText, subs: titleSubs } = blockTitle
...
function hardbreak (str, mark, ifAbsent = str, markIfAbsent = '', len = str.length) {
  return str[len - 1] === '+' && str[len - 2] === ' ' ? str.slice(0, len - 2) + mark : ifAbsent + markIfAbsent
}
