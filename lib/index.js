...
function downdoc (asciidoc, { attributes } = {}) {
  ...
  const refs = {}
  const substitutors = { quotes, attributes: subAttributeReferences.bind(attrs), macros: macros.bind(attrs), callouts }
      ...
      while (true) {
        ...
        if (skipping) {
          ...
          if (BLOCK_DELIMS[line] === 'verbatim') {
            ...
            inContainer = { delimiter: line, verbatim: !(inPara = false), subs: (subs = ['callouts']) }
            let style, lang
...
function callouts (str) {
  return str[str.length - 1] === '>' ? str.replace(ConumRx, (_, space, num) => space + CONUMS.get(num)) : str
}
