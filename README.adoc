...
** strong (bold), emphasis (italic), code phrase (monospace), literal code phrase, mark, span w/ line-through role, double curly quotes, curly apostrophe
** literal code phrase only escapes attribute references
...
To use a backtick in a code phrase in Markdown, it must be enclosed in backticks then enclosed in spaces.
